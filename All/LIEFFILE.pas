{============================
 ...
 Autor:       $Author: $
 Gelockt von: $Locker: $
 Originaldatum : Sat Jan 08 16:43:52 GMT+01:00 2000
...
TYPE TLIndex = ARRAY [1..LIndexNo] OF LAVL_Node;
     { Datentyp fÅr alle Indexknoten der Lieferdatenbank.
       Der Datentyp ist Basis der Recordstruktur von FLIndex }
     TLIndexType = ARRAY[1..LIndexNo] OF BYTE;
...
TYPE TLiefFilePtr = ^TLiefFile;
     TLiefFile = OBJECT
        ...
        FUNCTION GetTop(Kennung:BYTE;VAR Daten:TLiefdaten):BOOLEAN;
        FUNCTION GetBottom(Kennung:BYTE;VAR Daten:TLiefdaten):BOOLEAN;
        FUNCTION GetNext(Kennung:BYTE;VAR Daten:TLiefdaten):BOOLEAN;
        FUNCTION GetPrev(Kennung:BYTE;VAR Daten:TLiefdaten):BOOLEAN;
        FUNCTION StartIntervall(Kennung:BYTE; Anfang,Ende:STRING;VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION GetIntTop(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION GetIntBottom(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION GetIntNext(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION GetIntPrev(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION NewRec(Daten:TLiefDaten):BOOLEAN;
        FUNCTION GetRec(Kennung:BYTE;Schluessel:STRING;
                            VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION ChangeRec(Daten:TLiefDaten):BOOLEAN;
        FUNCTION DelRec(Schluessel:STRING):BOOLEAN;
        FUNCTION CloseFile:BOOLEAN;
        FUNCTION GetLfdLNr:STRING;
        PROCEDURE SetLfdLNr(Nr :STRING);
        FUNCTION MonthDBExists(Month: STRING):BOOLEAN;
        FUNCTION ChangeMonthDB(Month:STRING):BOOLEAN;
        FUNCTION IsOldDatabaseDay(Day:STRING):BOOLEAN;
        ...
        PRIVATE
        FUNCTION ContinueInNextDB(Daten:TLiefDaten):BOOLEAN;
        FUNCTION ContinueInPRevDB(Daten:TLiefDaten):BOOLEAN;
        FUNCTION ScanNextIntervallDB(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION ScanPrevIntervallDB(VAR Daten:TLiefDaten):BOOLEAN;
        FUNCTION TestIntervall:BOOLEAN;
        FUNCTION MakeIdxKey(Kennung:BYTE;Daten:TLiefDaten):STRING;
        FUNCTION GetDBFromKey(Kennung:BYTE;Schluessel:STRING):STRING;
        FUNCTION GetFirstKeyPart(Kennung:BYTE;Schluessel:STRING):STRING;
        ...
        PROCEDURE CreateNode(KeyVal:TLKeyType;Value:TLValType;
                                actNodeNo:LONGINT;VAR NewNode:LAVL_Node);
        PROCEDURE Insert(NewNode:LAVL_Node);
        PROCEDURE GetNode(NodeNo:LONGINT;VAR Node:LAVL_Node);
        FUNCTION GetRoot(VAR Node:LAVL_Node):BOOLEAN;
        PROCEDURE ChangeNode(AktNode:LAVL_Node);
        FUNCTION Search(KeyVal:TLKeyType;VAR Node:LAVL_Node):BOOLEAN;
        FUNCTION Delete(DelNode:LAVL_Node):BOOLEAN;
...
IMPLEMENTATION
   ...
   CONSTRUCTOR TLiefFile.OpenFile;
      ...
      BEGIN
         ...
         IF MultipleFiles THEN BEGIN
            IF (Datum <> '') THEN BEGIN
               ActualMonth := MakeMonthStr(Datum);
            END ELSE BEGIN
               IF (UPPER(COPY(Dateiname,1,4))<> 'LDB_') OR NOT
                  ValidMonth(COPY(Dateiname,4,4)) THEN BEGIN
                  ActualMonth := MakeMonthStr(ActDate);
               END ELSE
                  ActualMonth := COPY(Dateiname,4,4);
            END;
            StartMonth := ActualMonth;
            Dateiname := 'LDB_'+ActualMonth;
         END;
         ...
         IF NOT SeekFile(MakeFileName(ActualDB,'AVL')) THEN BEGIN
            ...
            IF (IORESULT = 0) THEN BEGIN
               InitTLiefDaten(Rec);
               Rec.LiefNr := '';
               FOR Lauf := 1 TO LIndexNo DO BEGIN
                  Index[Lauf].Key    := '';
                  Index[Lauf].Prev   := -1;
                  Index[Lauf].Next   := -1;
                  Index[Lauf].Weight := 0;
            ...
            END;
         END ELSE BEGIN
            IF (CorrectSize(MakeFileName(ActualDB,'AVL'),SizeOf(TLiefDaten))) AND
               (CorrectSize(MakeFileName(ActualDB,'IDX'),SizeOf(TLIndex))) AND
               (NumberOfRecs(MakeFileName(ActualDB,'AVL'),SizeOf(TLiefDaten)) =
                NumberOfRecs(MakeFileName(ActualDB,'IDX'),SizeOf(TLIndex))) THEN BEGIN
               {$I-}
                  IF MultipleFileAccess THEN BEGIN
                        CreateFLH;
                  END;
   ...
   FUNCTION TLiefFile.AssignFiles;
   ...
   PROCEDURE TLiefFile.CreateFLH;
      VAR MLauf : STRING;
          ...
          HDaten : TLiefDaten;
      BEGIN
         IF SEEKFile(DBDirectory+LHGFile) THEN BEGIN
            {$I-}
               RESET(FLH);
               SEEK(FLH,0);
               READ(FLH,MaxLief);
         ...
         WHILE Lauf > 0 DO BEGIN
            IF MonthDBExists(MLauf) THEN BEGIN
               {$I-}
               ASSIGN(HDB,DBDirectory+'LDB_'+MLauf+'.AVL');
               RESET(HDB);
               SEEK(HDB,0);
               READ(HDB,HDaten);
               CLOSE(HDB);
               {$I+}
               HLief := S2L(HDaten.LiefNr);
   ...
   PROCEDURE TLiefFile.ReOpenFile;
   ...
   PROCEDURE TLiefFile.SetTransActionOnly;
      BEGIN
   ...
   PROCEDURE TLiefFile.SetFreeAccess;
      BEGIN
         OnlyTransAction := FALSE;
         IF NOT IsOpen THEN
            ReOpenFile;
   ...
   FUNCTION TLiefFile.BeginTransAction;
      BEGIN
   ...
   FUNCTION TLiefFile.EndTransAction;
      BEGIN
   ...
   FUNCTION TLiefFile.GetTransActionError;
      BEGIN
         GetTransActionError := TransactionErrcode;
      END;

   FUNCTION TLiefFile.GetTransActionErrMsg;
      BEGIN
   ...
   FUNCTION TLiefFile.MonthDBExists;
      VAR Ergebnis : BOOLEAN;
          Dateiname : STRING;
      BEGIN
         Dateiname := DBDirectory+'LDB_'+Month;
   ...
   FUNCTION TLiefFile.NextMonthDB;
      VAR Ergebnis : BOOLEAN;
      BEGIN
         Ergebnis := FALSE;
         IF MultipleFileAccess THEN
   ...
   FUNCTION TLiefFile.PrevMonthDB;
      VAR Ergebnis : BOOLEAN;
      BEGIN
         Ergebnis := FALSE;
         IF MultipleFileAccess THEN
   ...
   FUNCTION TLiefFile.NextMonthDBStr;
      VAR Ergebnis : STRING;
          FMonth   : STRING;
          OneFound : BOOLEAN;
          FileInfo : SearchRec;
      CONST SeekPattern = 'LDB_*.AVL';
      BEGIN
         OneFound:= FALSE;
         Ergebnis:='';
   ...
   FUNCTION TLiefFile.PrevMonthDBStr;
      VAR Ergebnis : STRING;
          FMonth   : STRING;
          OneFound : BOOLEAN;
          FileInfo : SearchRec;
   ...
   FUNCTION TLiefFile.ChangeMonthDB;
      VAR Ergebnis : BOOLEAN;
      BEGIN
         Ergebnis := FALSE;
         IF MultipleFileAccess THEN BEGIN
            IF (Month <> ActualMonth) THEN BEGIN
               IF MonthDBExists(Month) THEN BEGIN
                  ...
                  IF Ergebnis THEN BEGIN
                     ActualMonth := Month;
                     Ergebnis :=AssignFiles('LDB_'+Month);
                     IF Ergebnis THEN
                        ReOpenFile;
                  END;
               END
   ...
   FUNCTION TLiefFile.IsOldDatabaseDay;
      VAR Ergebnis : BOOLEAN;
          Dateizugriff : BOOLEAN;
          DateIndex : STRING;
          Daten    : TLiefdaten;
      ...
      BEGIN
         ...
         ChkMonth := MakeMonthStr(Day);
         IF ChangeMonthDB(ChkMonth) THEN BEGIN
            DateIndex:= MakeDateIndex(Day,0);
            Dateizugriff := TRUE;
            IF  NOT GetRec(3,DateIndex,Daten) THEN
               Dateizugriff := GetNext(3,Daten);
            IF Dateizugriff THEN BEGIN
               IF (Daten.Liefertag = Day) THEN
                  Ergebnis := (Daten.TourNr < 50);
            END;
            Dateizugriff := ChangeMonthDB(SaveMonth);
         END;
         IsOldDataBaseDay := Ergebnis;
      END;

   FUNCTION TLiefFile.GetLfdLNr;
     VAR Dateizugriff : BOOLEAN;
         LHWert   : LONGINT;
         Ergebnis : STRING[5];
     ...
     BEGIN
       ...
       IF MultipleFileAccess THEN BEGIN
          ...
          Ergebnis := LeadingZeros(LHWert,5);
       END ELSE BEGIN
          Dateizugriff := ReadRec(0);
          Ergebnis := Rec.LiefNr;
       END;
   ...
   FUNCTION TLiefFile.CreateMonthDB;
      VAR Ergebnis : BOOLEAN;
          HDatei : FLiefStamm;
          HIdx : FLIndex;
          HRecFrei : FLiefRecs;
   ...
   FUNCTION TLiefFile.NewRec;
      ...
      BEGIN
         ...
         IF IsOpen THEN BEGIN
            IF MultipleFileAccess THEN BEGIN
               NRMonth := MakeMonthStr(Daten.Liefertag);
            ...
            END ELSE
               Ergebnis := TRUE;
            IF Ergebnis THEN BEGIN
               IF (NOT SeekRec(1,Daten.LiefNr)) THEN BEGIN
                  Schliessen := TRUE;
                  ...
                  END ELSE
                     Neu := FILESIZE(FDatei);
                  Rec :=  Daten;
                  FOR Lauf := 1 TO  LIndexNo DO
                     CreateNode(MakeIdxKey(Lauf,Daten),Neu,Neu,NewIdx[Lauf]);
                  Index := NewIdx;
                  Dateizugriff := WriteRec;
                  FOR Lauf := 1 TO LIndexNo DO BEGIN
                     IndexNo := Lauf;
   ...
   FUNCTION TLiefFile.MakeIdxKey;
      VAR Ergebnis : STRING;
      BEGIN
         Ergebnis := '';
         CASE Kennung OF
             1 : Ergebnis := UPPER(Daten.LiefNr);
             2 : Ergebnis := MakeKundIndex(Daten.Knr,Daten.Liefertag,
                                           Daten.TourNr);
             3 : Ergebnis := MakeDateIndex(Daten.Liefertag,Daten.TourNr);
             4 : Ergebnis := MakeDLfdIndex(Daten.Liefertag,Daten.TourNr,
                                           Daten.TourLfdNr);
             5 : Ergebnis := MakeFahrIndex(Daten.Fahrer,Daten.Liefertag,
                                           Daten.TourNr);
         END;
         MakeIdxKey := Ergebnis;
      END;

   FUNCTION TLiefFile.GETDBfromKey;
      VAR Ergebnis : STRING;
      BEGIN
         Ergebnis := '';
         CASE Kennung OF
             1 : Ergebnis := '';
             2 : Ergebnis := GetKundIndexMonth(Schluessel);

             3 : Ergebnis := GetDateIndexMonth(Schluessel);
             4 : Ergebnis := GetDLfdIndexMonth(Schluessel);
             5 : Ergebnis := GetFahrIndexMonth(Schluessel);
         END;
         GetDBFromKey := Ergebnis;
      END;

   FUNCTION TLiefFile.GetFirstKeyPart;
      VAR Ergebnis : STRING;
      BEGIN
         Ergebnis := '';
         CASE Kennung OF
            1 : Ergebnis := Schluessel;
            2 : Ergebnis := GetKundFromKey(Schluessel);
            3 : Ergebnis := Schluessel;
            4 : Ergebnis := GetDateFromKey(Schluessel);
            5 : Ergebnis := GetFahrFromKey(Schluessel);
         END;
         GETFirstKeyPart := Ergebnis;
      END;
   FUNCTION TLiefFile.MakeLowMonthKey;
      ...
      BEGIN
         ...
         CASE Kennung OF
            1 : Ergebnis := Schluessel;
            2 : Ergebnis := MakeKundIndex(GetFirstKeyPart(Kennung,Schluessel),
                                LowMonthDate(Month),0);
            3 : Ergebnis := MakeDateIndex(LowMonthDate(Month),0);
            4 : Ergebnis := MakeDLfdIndex(LowMonthDate(Month),0,0);
            5 : Ergebnis := MakeFahrIndex(GetFirstKeyPart(Kennung,Schluessel),
                                LowMonthDate(Month),0);
         END;
         MakeLowMonthKey := Ergebnis;
      END;

   FUNCTION TLiefFile.MakeHighMonthKey;
      VAR Ergebnis : STRING;
      BEGIN
         Ergebnis := '';
         CASE Kennung OF
            1 : Ergebnis := Schluessel;
            2 : Ergebnis := MakeKundIndex(GetFirstKeyPart(Kennung,Schluessel),
                                HighMonthDate(Month),999);
            3 : Ergebnis := MakeDateIndex(HighMonthDate(Month),999);
            4 : Ergebnis := MakeDLfdIndex(HighMonthDate(Month),999,999);
            5 : Ergebnis := MakeFahrIndex(GetFirstKeyPart(Kennung,Schluessel),
                                HighMonthDate(Month),999);
         END;
  ...
  FUNCTION TLiefFile.GetBottom;
    ...
    BEGIN
      ...
      IF GetTop(Kennung,Daten) THEN BEGIN
         ...
         Ergebnis := TRUE;
      END ELSE BEGIN
         InitTliefDaten(Daten);
      END;
      GetBottom := Ergebnis;
    END;

  FUNCTION TLiefFile.GetTop;
    VAR Dateizugriff : BOOLEAN;
   ...
   FUNCTION TLiefFile.StartIntervall;
      VAR   Ergebnis : BOOLEAN;
      ...
      BEGIN
         ...
         ScanDBChange := GETDBFromKey(ScanIdx,ScanStartKey) <>
                         GETDBFromKey(ScanIdx,ScanEndKey);
         Ergebnis :=GetIntTop(Daten);
         StartIntervall := Ergebnis;
      END;

   FUNCTION TLiefFile.GetIntTop;
      VAR Ergebnis : BOOLEAN;
          HMonth : STRING;
      BEGIN
         ...
         IF (IndexType[ScanIdx] = 2) THEN BEGIN
            IF NOT Ergebnis THEN BEGIN
               IF ScanForward THEN BEGIN
                  IF (GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) <
                     GetFirstKeyPart(ScanIdx,ScanStartKey)) OR
                    ((GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) =
                     GetFirstKeyPart(ScanIdx,ScanStartKey)) AND NOT(TestIntervall))
                     THEN BEGIN
                     Ergebnis := GetNext(ScanIdx,Daten);
                     Ergebnis := TestIntervall;
                  END
               END;
               IF (NOT Ergebnis) AND (NOT MonthDBExists(GetDBFromKey(ScanIdx,ScanStartKey))) AND
                  ScanDBChange THEN BEGIN
                  IF ScanForward THEN BEGIN
                     HMonth := NextMonthDBStr(GetDBFromKey(ScanIdx,ScanStartKey));
                     IF (HMonth <> '') AND (NOT (LaterMonth(Hmonth,
                           GetDBFromKey(Scanidx,ScanEndKey)))) THEN BEGIN
                     ...
                     END
                  END ELSE BEGIN
                     HMonth := PrevMonthDBStr(GetDBFromKey(ScanIdx,ScanStartKey));
   ...
   FUNCTION TLiefFile.GetIntBottom;
      VAR Ergebnis : BOOLEAN;
          HMonth : STRING;
      BEGIN
         ...
         IF (IndexType[ScanIdx] = 2) THEN BEGIN
            IF NOT Ergebnis THEN BEGIN
               IF (NOT ScanForward) THEN BEGIN
                  IF (GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) <
                     GetFirstKeyPart(ScanIdx,ScanEndKey))  OR
                    ((GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) =
                     GetFirstKeyPart(ScanIdx,ScanEndKey)) AND NOT(TestIntervall))
                     THEN BEGIN
                     Ergebnis := GetNext(ScanIdx,Daten);
                     Ergebnis := TestIntervall;
                  END
               END;
               IF (NOT Ergebnis) AND (NOT MonthDBExists(GetDBFromKey(ScanIdx,ScanEndKey))) AND
                  ScanDBChange THEN BEGIN
                  IF ScanForward THEN BEGIN
                     HMonth := PrevMonthDBStr(GetDBFromKey(ScanIdx,ScanEndKey));
                     IF (HMonth <> '') AND (NOT (EarlierMonth(Hmonth,
                           GetDBFromKey(Scanidx,ScanStartKey)))) THEN BEGIN
                        IF ChangeMonthDB(HMonth) THEN BEGIN
                           Ergebnis := GetRec(ScanIdx,MakeHighMonthKey(ScanIdx,
                     ...
                     END
                  END ELSE BEGIN
                     HMonth := NextMonthDBStr(GetDBFromKey(ScanIdx,ScanEndKey));
                     IF (HMonth <> '') AND (NOT (LaterMonth(Hmonth,
                           GetDBFromKey(Scanidx,ScanStartKey)))) THEN BEGIN
                        IF ChangeMonthDB(HMonth) THEN BEGIN
                           Ergebnis := GetRec(ScanIdx,MakeLowMonthKey(ScanIdx,
   ...
   FUNCTION TLiefFile.GetIntNext;
      ...
      BEGIN
         ...
         IF (ScanIdx <> 0) THEN BEGIN
            ...
            {ein weiterer Datensatz wurde gefunden, Intervall pr¸fen}
            IF Ergebnis THEN BEGIN
               Ergebnis := TestIntervall;
            END;
            {Wenn kein Datensatz mehr in der aktuellen DB gefunden wurde,
             oder der Datensatz nicht mehr im Intervall liegt }
   ...
   FUNCTION TLiefFile.ScanNextIntervallDB;
      VAR Ergebnis : BOOLEAN;
          DBExists : BOOLEAN;
          Month,
          LastMonth : STRING[4];
      ...
      BEGIN
        Ergebnis :=FALSE;
        DBExists := TRUE;
        LastMonth := GetDBFromKey(ScanIdx,ScanEndKey);
   ...
   FUNCTION TLiefFile.ScanPrevIntervallDB;
      VAR Ergebnis : BOOLEAN;
          DBExists : BOOLEAN;
          Month,
          FirstMonth : STRING[4];
      ...
      BEGIN
        Ergebnis :=FALSE;
        DBExists := TRUE;
        FirstMonth := GetDBFromKey(ScanIdx,ScanStartKey);
        WHILE ContinueInNextDB(Daten) AND (NOT Ergebnis) AND DBExists DO BEGIN
           IF ScanForward THEN BEGIN
              Month := PrevMonthDBStr(ActualMonth);
   ...
   FUNCTION TLiefFile.ContinueInNextDB;
      VAR Ergebnis : BOOLEAN;
          NothingFound : BOOLEAN;
          HData    : TLiefDaten;
      BEGIN
        InitTliefDaten(HData);
        NothingFound :=LGleich(HData,Daten);
        ...
        END ELSE BEGIN
           IF ScanForward THEN BEGIN
              Ergebnis := (GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) >
                          GetFirstKeyPart(ScanIdx,ScanEndKey)) AND
                          (EarlierMonth(GetDBFromKey(ScanIdx,Index[ScanIdx].Key),
                          GetDBFromKey(ScanIdx,ScanEndKey)))

           END ELSE BEGIN
              Ergebnis := (GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) <
                          GetFirstKeyPart(ScanIdx,ScanEndKey)) AND
                          (LaterMonth(GetDBFromKey(ScanIdx,Index[ScanIdx].Key),
                           GetDBFromKey(ScanIdx,ScanEndKey)))
           END;
        END;
   ...
   FUNCTION TLiefFile.ContinueInPrevDB;
      VAR Ergebnis : BOOLEAN;
          NothingFound : BOOLEAN;
          HData    : TLiefDaten;
      BEGIN
        ...
        IF NothingFound THEN BEGIN
           IF ScanForward THEN BEGIN
              Ergebnis := LaterMonth(ActualMonth,
                             GetDBFromKey(ScanIdx,ScanStartKey));
           END ELSE BEGIN
              Ergebnis := EarlierMonth(ActualMonth,
                             GetDBFromKey(ScanIdx,ScanStartKey));
           END;
        END ELSE BEGIN
           IF ScanForward THEN BEGIN
              Ergebnis := (GetFirstKeyPart(ScanIdx,Index[ScanIdx].Key) <
                          GetFirstKeyPart(ScanIdx,ScanStartKey)) AND
                          (LaterMonth(GetDBFromKey(ScanIdx,Index[ScanIdx].Key),
                          GetDBFromKey(ScanIdx,ScanStartKey)))
   ...
   FUNCTION TLiefFile.TestIntervall;
      VAR Ergebnis:BOOLEAN;
      BEGIN
         IF ScanForward THEN BEGIN
  ...
  FUNCTION TLiefFile.GetPrev;
   ...
   FUNCTION TLiefFile.ChangeRec;
      ...
      BEGIN
         ...
         SaveMonth := ActualMonth;
         IF MultipleFileAccess THEN BEGIN
            NewMonth :=MakeMonthStr(Daten.LieferTag);
            IF MonthDBExists(NewMonth) THEN
               Dateizugriff:= ChangeMonthDB(NewMonth);
            RecordFound := SeekRecMultiple(1,Daten.LiefNr);

         END ELSE
            RecordFound := SeekRec(1,Daten.LiefNr);
         IF RecordFound THEN BEGIN
            IF (Daten.TourLfdNr <> Rec.TourLfdNr) OR
               (Daten.TourNr <> Rec.TourNr) OR
               (Daten.Liefertag <> Rec.LieferTag) OR
               (Daten.Fahrer <> Rec.Fahrer) OR
               (Daten.Knr <> Rec.Knr) THEN BEGIN
               IF MultipleFileAccess AND (ActualMonth <> NewMonth) THEN BEGIN
                  Dateizugriff := DelRec(Daten.LiefNr);
                  Ergebnis := NewRec(Daten) AND Dateizugriff;
               END ELSE BEGIN
                  NewIdx := Index;
                  SaveIdx := Index;
                  FOR Lauf := 1 TO LIndexNo DO BEGIN
                      HIndexString := MakeIdxKey(Lauf,Daten);
                      IndexNo := Lauf;
   ...
   FUNCTION TLiefFile.SeekRecMultiple;
      VAR Pos          : LONGINT;
          Dateizugriff : BOOLEAN;
          Ergebnis     : BOOLEAN;
          SaveMonth    : STRING[4];
      ...
      BEGIN
         ...
         IF (IndexType[Kennung]> 0) THEN BEGIN
            Ergebnis := ChangeMonthDB(GETDBFromKey(IndexNo,Schluessel));
   ...
   PROCEDURE TLiefFile.CreateNode;
   ...
   PROCEDURE TLiefFile.ChangeNode;
   ...
   FUNCTION TLiefFile.Search;
      VAR AktNode       : LAVL_Node;
          HelpNode      : LAVL_Node;
          NodeNumber    : LONGINT;
   ...
   FUNCTION TLiefFile.Delete; {******* Lîschen eines Knotens ********}
      VAR RootNode,
          AktNode,
          HNode1,
          HNode2    : LAVL_Node;
      ...
      FUNCTION DeleteBin(AktNode:LAVL_Node;VAR FatherNode:LAVL_Node):INTEGER;

         VAR MinNode  : LAVL_Node;
             HelpNode : LAVL_Node;
             Ende : BOOLEAN;
             NewNodeNo   : LONGINT;
             OldNodeNo    : LONGINT;
         ...
         FUNCTION DeleteMin(RootNode:LAVL_Node;VAR ResultNode:LAVL_Node):INTEGER;
   ...
   PROCEDURE TLiefFile.Insert;
      ...
      PROCEDURE InsertBin(FirstNode:LAVL_Node;VAR NewNode:LAVL_Node);
         VAR AktNode    : LAVL_Node;
             Ende       : BOOLEAN;
             NextNodeNo : LONGINT;
             NewNodeNo  : LONGINT;
   ...
   PROCEDURE TLiefFile.LeftRotate;
      VAR SonNo,
          FatherNo   : LONGINT;
          AktNode,
          RightNode,
          LeftNode,
          FatherNode :LAVL_Node;
   ...
   PROCEDURE TLiefFile.RightRotate;
      VAR SonNo,
          FatherNo   : LONGINT;
          AktNode,
          LeftNode,
