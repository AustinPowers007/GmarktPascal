      ...
      BEGIN
         ...
         IF NOT LFOK THEN BEGIN
            FaultBox.Act(0,'Fehler beim Schreiben des OP-LogFiles '+
                           'OP-Listendarstellung unvollstÑndig, bitte'+
                           'manuell korregieren');
         END;
      END;
      ...
      BEGIN
         ...
         IF (NeueAnschrift) AND (Kunde.LieferANr = 0) THEN BEGIN
            ...
            IF NOT Valid AND (Kunde.KWerbKz = '') THEN
              FMessString:= 'Sie mÅssen eine Werbekennzeichen eingeben';
      ...
      BEGIN
         ...
         IF (Aendern AND Changed AND Request.Act('Achtung !!! Die '+
                    'aktuellen Aenderungen wurden nicht gesichert '+
                                ...
                                'bearbeiten wollen.'))
            OR (NOT Aendern AND
            Request.Act('Zum Anlegen einer zusÑtzlichen Adresse'+
               ...
               IF NOT NeueAnschrift THEN BEGIN
                  IF Request.Act('Wollen sie die Lieferanschrift '+Kunde.LiefStrasse+
                     ...
                     IF NOT LFehler THEN BEGIN
                        InfoBox.Act(2,'Lieferanschrift wurde gelîscht. ');
                  ...
                  FOR LoeschLauf := 9 DOWNTO 0 DO BEGIN
                     IF KundDb.GetRec(1,MakeLfdNr(LoeschKnr,LoeschLauf),Kunde) THEN BEGIN
                        IF NOT Kunddb.DelRec(MakeLfdNr(LoeschKnr,LoeschLauf)) THEN BEGIN
                           FaultBox.Act(0,'Fehler beim Lîschen von '+MakeLfdNr(LoeschKnr,LoeschLauf));
                           LFehler := TRUE;
                        END
                     END;
                  END;
                  IF NOT LFehler THEN
                     InfoBox.Act(2,'Kunde '+LeadingZeros(LoeschKNR,5)+' wurde gelîscht');
                  KundDB.EndTransAction;
                  Abbruch := TRUE;
               END;
            END;
   ...
   PROCEDURE InitKundenInput;
      ...
      BEGIN
         ...
         WITH KEditMenue^ DO BEGIN
            ...
            AddActionKey('_',27,ButtonBreak0,FALSE,'ZurÅck ins Hauptfenster');
            SetFKeyQuit(FALSE);
         END;
         ValTrue := TRUE;
         NamFeld :='';
         WITH Kunde DO BEGIN
            ...
            WITH KFenster^ DO BEGIN
               ...
               AddActionKey('_',-62,AdresseLoeschen,FALSE,'Lieferanschrift lîschen');
               AddActionKey('_',27,ZuLieferschein,FALSE,'Ohne Speichern in Lieferschein');
               FOR Lauf := 51 TO 58 DO
                  AddActionKey(B2s(Lauf,0),-62,WegBesLoeschen,FALSE,'Aktuelle Zeile lîschen');
               FOR Lauf := 61 TO 62 DO
                  AddActionKey(B2s(Lauf,0),-62,BemerkLoeschen,FALSE,'Aktuelle Zeile lîschen');
               ...
               AddActionKey('101',13,CrKFeld,FALSE,'Kunden Ñndern');
               AddActionKey('101',45,KFeldPlusMinus,FALSE,'Vorherige Kundennummer');
               AddActionKey('101',43,KFeldPlusMinus,FALSE,'NÑchste Kundennummer');
            ...
            WITH KAnlInfoFenster^  DO BEGIN
               ...
               FOR Lauf := 1 TO 4 DO BEGIN
                  AddString('',B2S(Lauf+3,0),'GesprÑch '+B2s(Lauf,0)+':',
               ...
               AlignFields;
               Disable('1');
               AddActionKey('_',27,AnlInfoEnde,TRUE,'ZurÅck ins Hauptfenster');
            ...
            WITH KOpostenFenster^ DO BEGIN
               ...
               FOR Lauf := 1 TO 10 DO BEGIN
                  ...
                  WITH Oposten[Lauf] DO BEGIN
                     ...
                     AddActionKey('0'+B2s(Lauf,0),-62,OpostenLoeschen,FALSE,'Aktuelle Zeile lîschen');
                  END;
            ...
            WITH KSTdFenster^ DO BEGIN
               ...
               FOR Lauf := 1 TO 5 DO BEGIN
                  ...
                  AddActionKey('0'+B2S(Lauf,0),-62,StdPosLoeschen,FALSE,'Aktuelle Position lîschen');
